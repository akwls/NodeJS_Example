...
router.route('/login').get(function(req, res) { //5
	console.log('/login 패스 요청됨.');
	res.render('login.ejs', {message: req.flash('loginMessage')});//6
});

// 사용자 인증 - POST로 요청받으면 패스포트를 이용해 인증함
// 성공 시 /profile로 리다이렉트, 실패 시 /login으로 리다이렉트함
// 인증 실패 시 검증 콜백에서 설정한 플래시 메시지가 응답 페이지에 전달되도록 함
router.route('/login').post(passport.authenticate('local-login', {  //1
    successRedirect : '/profile', 
    failureRedirect : '/login', //4
    failureFlash : true 
}));
...
router.route('/signup').get(function(req, res) {
	...
	res.render('signup.ejs', {message: req.flash('signupMessage')}); //s5
});

// 회원가입 - POST로 요청받으면 패스포트를 이용해 회원가입 유도함
// 인증 확인 후, 성공 시 /profile 리다이렉트, 실패 시 /signup으로 리다이렉트함
// 인증 실패 시 검증 콜백에서 설정한 플래시 메시지가 응답 페이지에 전달되도록 함
router.route('/signup').post(passport.authenticate('local-signup', {//s1
    successRedirect : '/profile', 
    failureRedirect : '/signup', //s4
    failureFlash : true 
}));
...
//패스포트 로그인 설정
passport.use('local-login', new LocalStrategy({ //a  //2
usernameField : 'email',
...
console.log('passport의 local-login 호출됨 : ' + email + ', ' + password);
			...
			database.UserModel.findOne({ 'email' :  email }, function(err, user) { //b  //user_schema.js 48line
						if (err) { return done(err); }

						// 등록된 사용자가 없는 경우
						if (!user) {
						console.log('계정이 일치하지 않음.');
						return done(null, false, req.flash('loginMessage', '등록된 계정이 없습니다.'));  //3
						// 검증 콜백에서 두 번째 파라미터의 값을 false로 하여 인증 실패한 것으로 처리
						}
						// 비밀번호 비교하여 맞지 않는 경우
						var authenticated = user.authenticate(password, user._doc.salt, user._doc.hashed_password);//user_schema.js 48line
						if (!authenticated) {
...
// 패스포트 회원가입 설정
passport.use('local-signup', new LocalStrategy({//a //s2
	usernameField : 'email',
	passwordField : 'password',
	passReqToCallback : true    // 이 옵션을 설정하면 아래 콜백 함수의 첫번째 파라미터로 req 객체 전달됨
	}, function(req, email, password, done) {
	...
	// findOne 메소드가 blocking되지 않도록 하고 싶은 경우, async 방식으로 변경
		process.nextTick(function() { //b
				var database = app.get('database');
				database.UserModel.findOne({ 'email' :  email }, function(err, user) { //c
				// 에러 발생 시
				if (err) {
					return done(err);
				}
				...
				if (user) { //d
					...
					return done(null, false, req.flash('signupMessage', '계정이 이미 있습니다.'));   //s3
					// 검증 콜백에서 두 번째 파라미터의 값을 false로 하여 인증 실패한 것으로 처리
				} else { 
					// 모델 인스턴스 객체 만들어 저장
